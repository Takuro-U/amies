...
const FilterBox: React.FC = () => {
    ...
    const setMargin = () => {
        Object.keys(checkLists).map((key) => {
            let widthDeltas: number[] = swiperRefs.current[key].map(
                //初期化
                (lst) => {
                    //各列の長さを計算
                    let w: number = 0;
                    if (lst.current)
                        lst.current.swiper.slidesSizesGrid.map((s) => {
                            w += s;
                        });
                    return w;
                }
            ...
            lastElemRefs.current[key].map((elem, i) => {
                if (elem.current)
                    elem.current.style.marginRight = widthDeltas[i] + "px";
            });
        });
    };
